...
void advection(real ***restrict q2,real ***u,real ***v,real ***w, real ***q1, real ***restrict uu,real ***restrict vv, real ***restrict ww,real dx,real dy,real dz,real dt,int i1,int i2,int j1,int j2,int k1,int k2,int nxdim,int nydim,int nzdim,real tstep, char advection_type)
{
    ...
    #pragma acc parallel vector_length(128) present(q2,u,q1) private (fld[0:nxdim], flux[0:nxdim]) 
    {
    ...
    #pragma acc parallel vector_length(128) present(q2,v,q1) private (fld[0:nydim], flux[0:nydim], qIn[0:nydim],qOut[0:nydim]) 
    {        
    ...
    #pragma acc parallel vector_length(128) present(q2,w,q1) private (fld[0:nzdim], flux[0:nzdim], qIn[0:nzdim],qOut[0:nzdim])
    {        
         // advection in z //
        #pragma acc loop gang collapse(2)
        for (int row=j1; row<=j2; ++row) {	
    ...
    #pragma acc parallel vector_length(128) present(u,v,w,uu,vv,ww) 
    {
